...
export default function Header(){const[wallet,setWallet]=useState(850000),[chargeOpen,setChargeOpen]=useState(false),[authOpen,setAuthOpen]=useState(false),path=window.location.pathname,accessToken=useSyncExternalStore(subscribeAccessToken,getAccessToken,getAccessToken),queryClient=useQueryClient(),logoutMutation=useMutation({...authMutations.logout(),onSettled:()=>{queryClient.removeQueries({queryKey:['auth']});window.history.pushState({},'','/');window.dispatchEvent(new PopStateEvent('popstate'))}});return <><header><div className="head-inner"><a className="logo" href="/" aria-label="홈으로 이동">KREAM</a><nav className="header-main-nav" aria-label="주요 메뉴">{mainNavigation.map(item=><a key={item.href} href={item.href} className={isActivePath(item.href,path)?'active':''} aria-current={isActivePath(item.href,path)?'page':undefined}>{item.label}</a>)}</nav><div className="head-account-actions"><button className="header-wallet" onClick={()=>setChargeOpen(true)}><Wallet/><span><small>내 전자지갑</small><strong>{wallet.toLocaleString()}P</strong></span><b>충전하기</b></button><nav className="header-account-nav" aria-label="계정 메뉴"><a href="/mypage" className={isActivePath('/mypage',path)?'active':''}>마이페이지</a>{accessToken?<button type="button" disabled={logoutMutation.isPending} onClick={()=>logoutMutation.mutate()}>{logoutMutation.isPending?'로그아웃 중...':'로그아웃'}</button>:<button type="button" onClick={()=>setAuthOpen(true)}>로그인</button>}</nav></div></div></header>{chargeOpen&&<WalletChargeDialog balance={wallet} onClose={()=>setChargeOpen(false)} onCharge={amount=>{setWallet(value=>value+amount);setChargeOpen(false)}}/>}<AuthModal open={authOpen} onClose={()=>setAuthOpen(false)}/></>}
