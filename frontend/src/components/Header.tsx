  ...
  const walletControl=authGate.status==='authenticated'
    ?walletQuery.isPending
      ...
      :wallet!==undefined
        ...
        :null
    :null;
  return <><header><div className="head-inner"><a className="logo" href="/" aria-label="홈으로 이동"><img src={dbiddingLogo} alt="DBIDDING"/></a><nav className="header-main-nav" aria-label="주요 메뉴">{mainNavigation.map(item=><a key={item.href} href={item.href} className={isActivePath(item.href,path)?'active':''} aria-current={isActivePath(item.href,path)?'page':undefined} onClick={event=>{if(item.requiresAuth&&!authGate.requestNavigation())event.preventDefault()}}>{item.label}</a>)}</nav><div className="head-account-actions">{walletControl}<NotificationBell/><nav className="header-account-nav" aria-label="계정 메뉴"><a href="/mypage" className={isActivePath('/mypage',path)?'active':''} onClick={event=>{if(!authGate.requestNavigation())event.preventDefault()}}>마이페이지</a>{accessToken?<button type="button" disabled={logoutMutation.isPending} onClick={()=>logoutMutation.mutate()}>{logoutMutation.isPending?'로그아웃 중...':'로그아웃'}</button>:<button type="button" onClick={()=>setAuthOpen(true)}>로그인</button>}</nav></div></div></header>{chargeOpen&&wallet!==undefined&&<WalletChargeDialog balance={wallet} onClose={()=>setChargeOpen(false)}/>}<AuthModal open={authOpen} onClose={()=>setAuthOpen(false)} onLoginSuccess={()=>setAuthOpen(false)}/></>;
}
