...
function NotificationItem({notification,onRead,onNavigate}:{notification:NotificationDto;onRead:(id:number)=>void;onNavigate:(notification:NotificationDto)=>void}){
  return <li className={`notification-item ${notification.isRead?'read':'unread'}`} onClick={()=>onRead(notification.id)}>
    <div className="notification-item-body">
      <p>{notification.message}</p>
      <span>{formatRelativeTime(notification.createdAt)}</span>
    </div>
    <button className="notification-item-move" onClick={event=>{event.stopPropagation();onNavigate(notification)}}>이동</button>
  </li>;
}
...
export default function NotificationBell(){
  ...
  const{isLoggedIn,notifications,unreadCount,isLoading,hasNextPage,isFetchingNextPage,fetchNextPage,refetchAll,markAsRead,markAllAsRead}=useNotifications(unreadOnly,isOpen);
  ...
  return <div className="notification-bell">
    ...
    {isOpen&&<div className="notification-backdrop" onMouseDown={event=>event.target===event.currentTarget&&setIsOpen(false)}>
      <section className="notification-drawer" role="dialog" aria-modal="true" aria-label="알림 목록">
