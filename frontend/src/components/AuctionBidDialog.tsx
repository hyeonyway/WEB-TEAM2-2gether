...
export default function AuctionBidDialog({auction,onClose}:{auction:AuctionDto;onClose:()=>void}){
  ...
  return createPortal(<div className="bid-backdrop" onMouseDown={event=>event.target===event.currentTarget&&onClose()}><section ref={dialogRef} className="bid-dialog" role="dialog" aria-modal="true" aria-label={`${auction.card.name} 경매 참여`}>
    ...
    <TabsContent value="bid">
      ...
      <div className="bid-options">{[minimum,minimum+bidIncrement*4,minimum+bidIncrement*9].map(value=><button key={value} className={amountValue===value?'active':''} disabled={closed||leading||bidMutation.isPending} onClick={()=>setManualAmount(value)}>{value.toLocaleString()}원</button>)}</div>
