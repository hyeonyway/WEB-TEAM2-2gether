...
import {activateAdminAccount, fetchAdminAccountWarnings, fetchAdminAccounts, suspendAdminAccount, type AdminAccountDto} from '../../api/adminAccountApi';
...
export default function AdminUsersPage() {
  ...
  const accounts = useQuery({queryKey: ['admin', 'users', page, keyword], queryFn: () => fetchAdminAccounts({page, size: pageSize, keyword}), enabled: authStatus === 'authenticated'});
  ...
  return <div className="cards-mypage standalone-dashboard"><Header/><main className="admin-users-page">
    <div className="admin-users-title"><div><small>ADMIN CONSOLE</small><h1>회원 관리</h1><p>회원 상태와 주문 취소 경고 이력을 확인하고 이용 제한을 관리합니다.</p></div><nav aria-label="관리자 메뉴"><Link className="active" to="/admin/users">회원 관리</Link><Link to="/admin/stream-recovery">Stream 복구</Link></nav></div>
    <form className="admin-users-search" onSubmit={event => {event.preventDefault(); search();}}><label htmlFor="admin-user-search">회원 검색</label><input id="admin-user-search" value={keywordInput} onChange={event => setKeywordInput(event.target.value)} placeholder="이메일 또는 닉네임"/><button type="submit">검색</button></form>
    <section className="admin-users-list" aria-label="회원 목록"><div className="admin-users-list-head"><div><h2>회원 목록</h2><p>이메일과 닉네임으로 회원을 검색할 수 있습니다.</p></div><span>전체 {data.total_elements.toLocaleString()}명</span></div>
      {data.content.length ? <><div className="admin-users-table" role="table"><div className="admin-users-row admin-users-head" role="row"><span>회원</span><span>상태</span><span>활성 경고</span><span>가입일</span><span>관리</span></div>{data.content.map(account => <div className="admin-users-row" role="row" key={account.id}><div><b>{account.nickname}</b><small>{account.email} · {account.role}</small></div><span><b className={`admin-status ${account.status.toLowerCase()}`}>{account.status}</b></span><div>{account.active_warning_count > 0 ? <><b className="admin-warning">활성 경고 {account.active_warning_count}건</b><small>만료 {toKoreanDate(account.latest_active_warning_expires_at)}</small></> : <span>-</span>}</div><time>{toKoreanDate(account.created_at)}</time><div className="admin-user-actions"><button type="button" onClick={() => setWarningsFor(account)}>경고 이력</button>{account.status !== 'WITHDRAWN' && <button type="button" className={account.status === 'ACTIVE' ? 'danger' : ''} onClick={() => {setWarningsFor(null); setActionTarget(account);}}>{account.status === 'ACTIVE' ? '정지' : '활성화'}</button>}</div></div>)}</div>{data.total_pages > 1 && <div className="admin-users-pagination"><button type="button" disabled={page === 0} onClick={() => setPage(value => value - 1)}>이전</button><span>{page + 1} / {data.total_pages}</span><button type="button" disabled={page + 1 >= data.total_pages} onClick={() => setPage(value => value + 1)}>다음</button></div>}</> : <div className="admin-users-no-results">검색 조건에 맞는 회원이 없습니다.</div>}
    </section>
  </main>
  {warningsFor && <div className="admin-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setWarningsFor(null)}><section className="admin-modal" role="dialog" aria-modal="true" aria-label={`${warningsFor.nickname} 경고 이력`}><h2>{warningsFor.nickname} 경고 이력</h2>{warnings.isPending ? <p>경고 이력을 불러오는 중입니다.</p> : warnings.isError ? <><p className="admin-modal-error">{errorMessage(warnings.error, '경고 이력을 불러오지 못했습니다.')}</p><button type="button" onClick={() => void warnings.refetch()}>다시 시도</button></> : warnings.data?.length ? <ul>{warnings.data.map(warning => <li key={warning.id}><b>{warning.reason}</b><span>주문 #{warning.order_id} · {toKoreanDate(warning.issued_at)} 발행 · {toKoreanDate(warning.expires_at)} 만료</span></li>)}</ul> : <p>등록된 경고 이력이 없습니다.</p>}<div className="admin-modal-actions"><button type="button" onClick={() => setWarningsFor(null)}>닫기</button></div></section></div>}
