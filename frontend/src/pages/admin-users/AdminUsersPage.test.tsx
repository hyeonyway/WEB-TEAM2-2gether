...
const apiMocks = vi.hoisted(() => ({
  fetchAdminAccounts: vi.fn(), fetchAdminAccountWarnings: vi.fn(), suspendAdminAccount: vi.fn(), activateAdminAccount: vi.fn(),
}));
...
describe('AdminUsersPage', () => {
  beforeEach(() => {
    ...
    apiMocks.fetchAdminAccountWarnings.mockReset().mockResolvedValue([{id: 3, order_id: 12, reason: 'ORDER_CANCELLATION', issued_at: '2026-08-10T00:00:00Z', expires_at: '2026-09-10T00:00:00Z'}]);
  ...
  it('회원 목록의 상태와 활성 경고를 표시하고 검색한다', async () => {
    ...
    await waitFor(() => expect(apiMocks.fetchAdminAccounts).toHaveBeenLastCalledWith({page: 0, size: 20, keyword: '피카'}));
  });
  ...
  it('경고 이력을 열고 확인 후 회원을 정지한다', async () => {
    ...
    expect(await screen.findByText('ORDER_CANCELLATION')).toBeInTheDocument();
