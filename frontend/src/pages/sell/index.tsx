...
import{fetchPsaCertificationSample,lookupPsaCertification,scanCardImage}from'../../api/sellApi';
...
export default function SellPage({Header}){
  ...
  const applyPsaCardSelection=(detail:CardDetailResponseDto,certification:{psaGrade:string;population:string})=>{
    setCardVariants([]);
    setCardResults([]);
    setCardSearchStatus('idle');
    setPsaMismatch(false);
  ...
  return <div className="sell-page">{Header&&<Header/>}<main><SellPageHeader/>
    ...
    <section className="sell-step-card">
      {step===1&&<StepOne form={form} setField={setField} panel={panel} setPanel={setPanel} ocrFile={ocrFile} chooseOcr={chooseOcr} scan={scan} ocrStatus={ocrStatus} psaNumber={psaNumber} setPsaNumber={value=>{const next=digits(value);psaNumberRef.current=next;psaLookupSequence.current++;setPsaNumber(next);setPsaStatus('idle')}} validPsa={validPsa} lookupPsa={lookupPsa} fillSamplePsa={fillSamplePsa} psaStatus={psaStatus} validYear={validYear} validPopulation={validPopulation} selectedCard={selectedCard} psaMismatch={psaMismatch} cardResults={groupCards(cardResults)} cardSearchStatus={cardSearchStatus} clearSelectedCard={clearSelectedCard} resetCardSelection={resetCardSelection} selectCard={selectCard} selectSelfGrade={selectSelfGrade} selectGradeType={selectGradeType} psaVerified={psaVerified} cardNameInputRef={cardNameInputRef}/>}
...
function StepOne({form,setField,panel,setPanel,ocrFile,chooseOcr,scan,ocrStatus,psaNumber,setPsaNumber,validPsa,lookupPsa,fillSamplePsa,psaStatus,validYear,validPopulation,selectedCard,psaMismatch,cardResults,cardSearchStatus,clearSelectedCard,resetCardSelection,selectCard,selectSelfGrade,selectGradeType,psaVerified,cardNameInputRef}){
