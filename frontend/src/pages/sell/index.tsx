...
const MAX_START_PRICE=999_000_000_000;
const MAX_BID_INCREMENT=1_000_000_000;
const moneyDigits=(value,max=MAX_MONEY)=>{
  const raw=digits(value);
  return raw&&Number(raw)>max?String(max):raw;
};
...
function MoneyInput({id,value,onChange,max=MAX_MONEY}){return <div className="sell-money-input"><input id={id} inputMode="numeric" value={money(value)} onChange={e=>onChange(moneyDigits(e.target.value,max))}/><span>원</span></div>}
