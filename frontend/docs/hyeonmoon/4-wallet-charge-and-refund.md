...
- `INSUFFICIENT_AVAILABLE_BALANCE`는 최신 잔액을 다시 조회하고 같은 키를 유지한다.
- `IDEMPOTENCY_CONFLICT`는 입력을 유지하되 새 키를 발급해 명시적으로 재시도한다.

## 충전 UI

- 최소 1,000원을 프론트에서도 검증한다.
- 충전 금액은 0원에서 시작하며 직접 입력과 금액 추가 버튼은 하나의 `amount`
  state를 사용한다.
- `+5만원`, `+10만원`, `+30만원`은 금액을 고정 선택하지 않고 현재 입력값에
  누적한다. 예를 들어 20만원 입력 후 `+5만원`을 누르면 25만원이 된다.
- 충전 모달에도 현재 총 잔액·동결 금액·가용 잔액을 함께 표시한다.
- 충전 예정 금액은 총 잔액과 가용 잔액에 더하고 동결 금액은 바꾸지 않은 상태로
  충전 후 예상값을 안내한다.
- 입력값이나 충전 후 예상 잔액이 JavaScript 안전 정수 범위를 벗어나면 계산값을
  표시하거나 요청하지 않는다.
...
| 409, `INSUFFICIENT_AVAILABLE_BALANCE` | Wallet Query 재조회, 입력·멱등키 유지 |
| 409, `IDEMPOTENCY_CONFLICT` | 입력 유지, 새 멱등키 발급 |
| 그 밖의 409 | 입력·멱등키 유지, 일반 실패 안내 |
| 네트워크 오류 | 입력과 멱등키를 유지하고 명시적 재시도 제공 |
| 5xx | 입력 유지, 자동 반복 요청 금지 |

프론트는 `HttpError.code`만 분기하고 백엔드의 한국어 메시지 문자열은 파싱하지
않는다.

## 필수 테스트

- 충전 1,000원 미만과 환불 0원 이하는 요청하지 않는다.
- 충전 금액 추가 버튼은 현재 직접 입력값에 누적된다.
- 충전 모달은 Wallet snapshot의 총 잔액·동결 금액·가용 잔액을 구분해 표시한다.
- 환불 가용액 초과를 안내한다.
- 같은 거래의 네트워크 재시도는 같은 Idempotency-Key를 사용한다.
- 새 거래는 이전 거래와 다른 키를 사용한다.
- 제출 중 중복 클릭이 요청을 늘리지 않는다.
- 성공 뒤 Wallet Balance Query를 무효화한다.
- 잔액 부족 409 뒤 Wallet Query를 무효화하고 같은 멱등키를 유지한다.
- 멱등키 충돌 409 뒤 입력값을 유지하고 새 멱등키를 사용한다.
- 네트워크 오류 뒤 입력값과 멱등키를 유지한다.
...
- [x] 금액 추가 버튼이 직접 입력값에 누적되고 변경 시 새 멱등키를 사용한다.
- [x] 충전 모달에서 현재 Wallet 잔액 3종과 충전 후 예상값을 확인할 수 있다.
- [x] Wallet 409 오류 코드를 구분해 잔액 부족과 멱등키 충돌을 다르게 처리한다.
