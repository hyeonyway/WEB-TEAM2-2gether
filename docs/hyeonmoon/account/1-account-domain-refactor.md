...
- [ ] **Step 1: Account 생성 규칙 테스트를 작성한다**
...
- [ ] **Step 1: AuthService 테스트를 AccountRepository 계약으로 변경한다**
...
- [ ] **Step 2: 기존 AuthService에서 변경된 테스트가 실패하는지 확인한다**
...
- [ ] **Step 3: AuthService의 회원가입·로그인·재발급을 AccountRepository에 연결한다**
...
- [ ] **Step 4: UNIQUE 충돌 예외 계약을 AuthService에 유지한다**
...
- [ ] **Step 5: SignupResponse와 JWT 역할 타입을 Account로 변경한다**
...
- [ ] **Step 6: UserAccount Port·Adapter와 기존 User 모델을 제거한다**

새 Account 모델로 대체된 소스와 테스트만 삭제한다. WalletProvisioningPort는 다음 Task까지 현재 위치에 유지한다.

- [ ] **Step 7: Auth와 Account 테스트를 통과시키고 커밋한다**
...
- [ ] **Step 1: Account 패키지 경계 검증을 추가한다**
...
- [ ] **Step 2: Auth 소스와 테스트를 Account 아래로 이동한다**
...
- [ ] **Step 3: Global JWT 연결 import를 변경한다**
...
- [ ] **Step 4: Wallet provisioning Port import를 변경한다**
...
- [ ] **Step 1: 회원가입 원자성과 인증 회귀 테스트를 실행한다**
...
- [ ] **Step 5: 문서와 이슈 체크리스트를 완료 상태로 갱신한다**
