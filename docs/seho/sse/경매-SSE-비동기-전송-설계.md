...
SSE 이벤트에는 애플리케이션 내부의 단조 증가 ID를 부여한다. 프론트는 이 ID로
캐시에 이미 반영된 이벤트보다 늦게 도착한 이벤트를 버린다. 서버는 이벤트 이력이나
경매별 최신 상태를 저장하지 않으며 `Last-Event-ID`를 처리하지 않는다.

재연결이 성공하면 공유 `EventSource`가 한 번만 재연결 신호를 발행한다. 활성 화면은
해당 신호에서 REST query를 무효화해 현재 경매 상태를 다시 조회한다. 최초 진입 역시
REST 조회가 기준이며, SSE는 연결 중 발생한 변경을 즉시 반영하는 역할만 담당한다.
...
`POST /api/auctions/stream/test-events/random-bid`를 호출한다. 재연결 후 활성 화면의
REST 요청이 다시 발생하고 최신 상태가 표시되는지 Network 탭에서 확인할 수 있다.

이벤트 유실을 허용하는 `DiscardPolicy`를 사용하지 않는다. 포화 시
