...
1. `item_daily_statistics`에서 오늘을 제외한 최근 30일의 카드별 유효 거래를 조회한다.
2. `latest_price`가 null 또는 0인 날짜는 유효 거래에서 제외한다. `average_price`는 대체 가격으로 사용하지 않는다.
3. 유효 거래가 2개 미만인 카드는 가격 변동 통계에서 제외한다.
4. 가장 최근 유효 거래일의 가격을 현재가, 그 직전 유효 거래일의 가격을 이전가로 비교한다.
5. 양수 변동은 `gainers`, 음수 변동은 `losers`에 담는다.
6. 상승은 변동률 내림차순, 하락은 변동률 오름차순으로 정렬한다.
