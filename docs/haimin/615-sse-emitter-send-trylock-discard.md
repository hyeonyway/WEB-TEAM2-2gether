...
**#614 수정 계획에 대한 함의(최초 판단, 아래에서 정정됨)**: 지금은 nginx가 (의도한
건 아니지만) 60초마다 느린 클라이언트를 정리해주는 안전망 역할을 하고 있다.
`proxy_buffering off`만 켜서 이 버그를 고치면 그 안전망이 사라지고, 백엔드가
직접 몇 분~몇십 분 단위로 스레드를 붙잡히는 상황에 노출된다 — threadpool
프로필이면 최악의 경우 스레드풀 전체가 소수의 느린 클라이언트에 다 묶일 수
있다. #614를 고칠 때 nginx의 `proxy_buffering off`만이 아니라, 백엔드 쪽에도
"일정 시간 진행이 없으면 emitter를 강제로 끊는" 메커니즘이 같이 필요할 수
있다고 판단했었다.

### 정정 — 실측 확인 결과, 이 우려는 과했다 (2026-08-20)

위 판단은 "버퍼링을 끄면 안전망이 통째로 사라진다"는 전제였는데, 이 전제가
틀렸다. **`send_timeout`(클라이언트에 마지막으로 쓴 뒤 60초 안에 못 내보내면
끊는 것)은 `proxy_buffering`과 완전히 별개의 설정**이라, 버퍼링을 꺼도 그대로
살아있다. `proxy_read_timeout`/`proxy_send_timeout`(prefix 있음, 1시간으로
설정돼있는 그것)은 nginx↔**백엔드** 사이 타임아웃이라 이 얘기와 무관하다 —
헷갈리기 쉬운 부분이라 명시해둔다.

**직접 재현해서 확인**: nginx location을 실제 컨트롤러 경로(`/api/auctions/stream`,
`/api/me/stream`)에 맞게 임시로 고치고(`proxy_buffering off` 포함, #614 수정
방향과 동일), 완전히 죽은 클라이언트(헤더만 읽고 이후 `recv()` 전혀 안 함)를
nginx(443)를 거쳐 연결한 뒤, 이 클라이언트를 향한 이벤트를 계속 발행하면서
`dbidding_sse_connections_closed_total{reason="send_failure",stream="auction"}`
메트릭을 관찰했다(백엔드를 재시작해 다른 테스트 잔재 없는 깨끗한 상태에서
측정). 결과: **53초 지점엔 0, 68초 지점엔 1** — `send_timeout`(60초) 시점과
정확히 맞아떨어졌다. nginx가 60초 뒤 이 클라이언트와의 연결을 끊으면, 그
요청과 짝지어진 백엔드 쪽 연결도 같이 정리되면서(nginx가 업스트림 연결도
같이 닫음) 백엔드의 블로킹된 `emitter.send()`도 그 직후 `IOException`으로
풀려난다 — 리눅스 TCP 재전송 타임아웃(15~20분)까지 안 기다려도 된다.

**정정된 결론**: 완전히 죽은(응답 0인) 클라이언트에 대한 60초 컷오프는
`#614`(버퍼링 버그)를 고쳐도 **자동으로 유지된다** — 위에서 우려했던 "안전망이
통째로 사라진다"는 판단은 틀렸다. 여전히 남는 진짜 위험은 **"느리지만 완전히
죽진 않은"** 클라이언트뿐이다 — 어쩌다 한 번씩이라도 몇 바이트를 읽어가면
그때마다 `send_timeout` 타이머가 리셋되니, 이론상 영원히 살아있을 수 있다.
근데 이건 현실 트래픽에서 거의 안 나오는 패턴이라 별도 워치독까지는 필요
없어 보인다 — **#614는 nginx `location` 매칭만 고치면 충분하다.**
