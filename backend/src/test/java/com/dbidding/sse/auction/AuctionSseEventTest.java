...
class AuctionSseEventTest {
    ...
    void 생성_이벤트를_전체_렌더링_payload로_직렬화한다() throws Exception {
        var payload = new AuctionCreatedPayload(
                ...
                40_000L, 40_000L, 1_000L, 0, now.plusSeconds(3600),
        ...
        assertThat(json.get("ends_at").asText()).isEqualTo("2026-07-30T13:00:00Z");
        assertThat(json.get("occurred_at").asText()).isEqualTo("2026-07-30T12:00:00Z");
    }
    ...
    void 입찰_이벤트에_현재가와_이전_입찰자를_포함한다() throws Exception {
        var payload = new BidPlacedPayload(
                null, 10, 7, 5,
                40_000L, 50_000L, 1_000L, 2, now.plusSeconds(3600),
