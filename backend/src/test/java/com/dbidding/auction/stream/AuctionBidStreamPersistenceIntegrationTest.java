...
class AuctionBidStreamPersistenceIntegrationTest {
    ...
    void 상회입찰과_즉시낙찰_이벤트가_기존_DB_상태전이를_재현한다() {
        ...
        assertThat(jdbcTemplate.queryForObject(
                "SELECT event_type FROM auction_timeline_events WHERE stream_id = '1-0'", String.class
        )).isEqualTo("bid.accepted.v1");
        assertThat(jdbcTemplate.queryForObject(
                "SELECT payload FROM auction_timeline_events WHERE stream_id = '1-0'", String.class
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM auction_timeline_events WHERE auction_id = ?", Integer.class, AUCTION_ID))
                .isEqualTo(2);
    ...
    void Redis가_발급한_경매_ID로_생성_event를_MySQL에_projection한다() {
        ...
        assertThat(jdbcTemplate.queryForObject("SELECT auction_id FROM auction_timeline_events WHERE stream_id = 'created-1'", Integer.class))
