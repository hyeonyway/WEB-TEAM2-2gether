...
public sealed interface AuctionWalletTimelineEvent permits BidAcceptedStreamEvent, WalletChargedStreamEvent, WalletStateChangedStreamEvent {
    String streamId();
    ...
    static AuctionWalletTimelineEvent from(String streamId, Map<String, String> values) {
        ...
        if ("wallet.charged.v1".equals(eventType)) {
            return WalletChargedStreamEvent.from(streamId, values);
        }
