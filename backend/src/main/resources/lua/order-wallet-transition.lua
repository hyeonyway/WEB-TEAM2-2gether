...
local maxBalance = tonumber(ARGV[10])
if not maxBalance or price > maxBalance then return 'REJECTED|AMOUNT_LIMIT_EXCEEDED' end
...
if nextAvailable + frozen > maxBalance then return 'REJECTED|BALANCE_LIMIT_EXCEEDED' end
local walletVersion = redis.call('HINCRBY', KEYS[2], 'walletVersion', 1)
local orderVersion = redis.call('HINCRBY', KEYS[1], 'orderVersion', 1)
local priceText = integerString(price)
local nextAvailableText = integerString(nextAvailable)
local frozenText = integerString(frozen)
local walletVersionText = integerString(walletVersion)
local orderVersionText = integerString(orderVersion)
redis.call('HSET', KEYS[2], 'availableBalance', nextAvailableText, 'frozenBalance', frozenText)
...
local streamId = redis.call('XADD', KEYS[4], '*',
    ...
    'orderId', ARGV[4], 'auctionId', ARGV[5], 'orderVersion', orderVersionText,
    'actorId', ARGV[1], 'buyerId', buyerId, 'sellerId', sellerId, 'status', ARGV[2],
    'walletUserId', walletUserId, 'walletVersion', walletVersionText,
    'availableBalance', nextAvailableText, 'frozenBalance', frozenText,
    'transactionType', transactionType, 'transactionAmount', priceText,
    'idempotencyKey', ARGV[6], 'occurredAt', ARGV[9])
local result = 'ACCEPTED|' .. streamId .. '|' .. ARGV[2] .. '|' .. orderVersionText .. '|' .. walletVersionText
    .. '|' .. nextAvailableText .. '|' .. frozenText .. '|' .. walletUserId
